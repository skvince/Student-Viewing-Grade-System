  ...
  $studentSaveError = '';
    ...
    if (isset($_GET['edit_student'])) {
      ...
      if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_student'])) {
        ...
        if ($studentIdToDelete) {
          ...
          audit_log('delete_student');
            ...
            // Keep page to show error/success state; helps debugging insert problems.
            //header('Location: ' . $_SERVER['PHP_SELF']);
            //exit;
      ...
      if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_student') {
        ...
        $studentSaveError = $studentSaveError ?: 'DEBUG: add_student handler reached';
        ...
        if (! $firstName || ! $lastName) {
          ...
          // DEBUG: show create_student() return payload so we can see why insert fails.
          // If it succeeds, the page redirects and we won't see this.
          if (is_array($result) && isset($result['success']) && !$result['success']) {
            $studentSaveError = 'DEBUG create_student() return: ' . htmlspecialchars(print_r($result, true));
          }
          ...
          if (!is_array($result) || empty($result['success'])) {
            ...
            if (is_array($result)) {
              $studentSaveError = $result['error'] ?? '';
              if (trim($studentSaveError) === '') {
                $studentSaveError = 'Student save failed (no error message returned).';
              }
            } else {
              $studentSaveError = 'Student save failed.';
            }
          } else {
      ...
      <header class="mobile-header">
        ...
        <div class="main-content">
          ...
          <div class="tab-content" style="display: block;">
            ...
            <!-- Student created banner intentionally removed -->
            <?php endif; ?>
